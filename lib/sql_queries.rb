...
def selects_oldest_bear_and_returns_name_and_age
  "SELECT name, MAX(age) FROM bears ORDER BY name;"
end

def select_youngest_bear_and_returns_name_and_age
  "SELECT name, MIN(age) FROM bears ORDER BY name;"
end

def selects_most_prominent_color_and_returns_with_count
  "SELECT color, COUNT(color) FROM bears GROUP BY color ORDER BY COUNT(*) DESC LIMIT 1;"
#First we got the color count, then group by color, then order by count(all) decen choose the first one
end
